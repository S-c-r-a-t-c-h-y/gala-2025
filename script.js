...
const sections = Array.from(
  document.querySelectorAll(
    "#bienvenue, #inscription, #menu, #planning, #partenaire"
  )
);

document.addEventListener("scroll", function () {
  const blocks = document.querySelectorAll(".planningblock");
  ...
  blocks.forEach((block) => {
    ...
    if (blockTop < triggerPoint) {
      // Start the animation by resuming play-state
...
$(document).ready(function () {
  ...
  var q_array = [
    ...
    tw_array = [
      " <strong>TW</strong> : Alcool ",
      " <strong>TW</strong> : Contact physique",
      " <strong>TW</strong> : Mégenrage",
      " <strong>TW</strong> : Témoin de VSS ",
      " <strong>TW</strong> : Mention de rapport sexuel et consentement ",
    ],
    ex_array = [
  ...
  var right = function (question, answer) {
    ...
    for (var i = 0; i < question.length; i++) {
      input_tw =
        "<p>" +
        tw_array[i] +
        '<br/> </p> <div class="unblur' +
        i.toString() +
        ' buttonblur">Révéler </div>';
      li = $("<li>");
      ...
      input =
        "<p> Explication: " +
        ex_array[i] +
        ' </p><h2 class="w-full md:w-1/2 mx-auto text-center text-4xl font-bold text-primary mb-4 border-b-4 pb-4 border-primary" style=" border-radius: 0px"></h2>';
      li.append(input);
      ul.append(li);
    }
    div.append(ul);
  ...
  var display = function () {
    if (display.index < q_array.length) {
      ...
      // ! CHANGER ICI LA COULEUR POUR LES QUESTIONS
      var paraq = $('<p style = "background : #c2a87d">').append(
  ...
  $(".lien").on("click", function () {
    window.location.href =
      "https://www.helloasso.com/associations/bde-ensl/evenements/gala-2025";
  });
